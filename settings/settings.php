...
if (!isset($_SESSION["mikhmon"])) {
  ...
  if ($id == "settings" && explode("-",$router)[0] == "new") {
    ...
    fwrite($f, "\n'$'data['".$router."'] = array ('1'=>'".$router."!','".$router."@|@','".$router."#|#','".$router."%','".$router."^','".$router."&Rp','".$router."*10','".$router."(1','".$router.")','".$router."=','".$router."@!@disable','".$router."<','".$router."?','".$router."|?disable','".$router."#!#');");
    fclose($f);
    $search = "'$'data";
    $replace = (string)"$data";
  ...
  if (isset($_POST['save'])) {
    ...
    $sincomerecap = ($_POST['incomerecap']);
    $sinfolog = "";

    $search = array('1' => "$session!$iphost", "$session@|@$userhost", "$session#|#$passwdhost", "$session%$hotspotname", "$session^$dnsname", "$session&$currency", "$session*$areload", "$session($iface", "$session)$maxtx", "$session=$maxrx", "'$session'", "$session@!@$livereport", "$session<$token", "$session?$cid", "$session|?$incomerecap", "$session#!#$infolog");
    $replace = array('1' => "$sesname!$siphost", "$sesname@|@$suserhost", "$sesname#|#$spasswdhost", "$sesname%$shotspotname", "$sesname^$sdnsname", "$sesname&$scurrency", "$sesname*$sreload", "$sesname($siface", "$sesname)$smaxtx", "$sesname=$smaxrx", "'$sesname'", "$sesname@!@$slivereport", "$sesname<$stoken", "$sesname?$scid", "$sesname|?$sincomerecap", "$sesname#!#$sinfolog");
  ...
  <div class="col-12">
    <div class="card">
      ...
      <div class="card-body">
        <table class="table">
          <tr>
            <td>Token Bot<td><textarea class="form-control" name="token" title="Token bot Telegram" cols="10" rows="2" placeholder="xxxxxxxxx:ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghi"><?php echo $token; ?></textarea></td></td>
          </tr>
          <tr>
            <td class="align-middle">Chat id</td><td><input class="form-control" type="text" name="cid" title="Chat id Telegram" placeholder="-1234567890" value="<?php echo $cid; ?>"/></td>
          </tr>
        </table>
      </div>
    </div>
...
<?php if (empty($incomerecap) && empty($token) && empty($cid)) {
} else { ?>
  <tr>
    <td><?= $_income_recap ?></td>
    <td>
      <select class="form-control" name="incomerecap" >
          <?php if(empty($incomerecap)):?>
            <option value="disable>">Disable</option>
          <?php else :?>
            <option value="<?= $incomerecap; ?>"><?= ucfirst($incomerecap); ?></option>
          <?php endif ?>
				  <option value="enable">Enable</option>
				  <option value="disable">Disable</option>
		  </select>
